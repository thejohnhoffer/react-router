 ...
 */
export function MemoryRouter({ basename, children, initialEntries, initialIndex }) {
 ...
 */
export function Router({ basename: basenameProp = "/", children = null, location: locationProp, navigationType = NavigationType.Pop, navigator, static: staticProp = false }) {
    ...
    let navigationContext = React.useMemo(() => ({ basename, rootPath, navigator, static: staticProp }), [rootPath, basename, navigator, staticProp]);
    ...
    return (React.createElement(NavigationContext.Provider, { value: navigationContext },
        React.createElement(LocationContext.Provider, { children: children, value: { location, navigationType } })));
}
/**
...
export function useNavigate() {
    ...
    let navigate = React.useCallback((to, options = {}) => {
        warning(activeRef.current, `You should call navigate() in a React.useEffect(), not when ` +
        ...
        let path = resolveTo(to, JSON.parse(routePathnamesJson), locationPathname, rootPath);
...
export function useResolvedPath(to) {
    ...
    let routePathnamesJson = JSON.stringify(matches.map(match => match.pathnameBase));
    return React.useMemo(() => resolveTo(to, JSON.parse(routePathnamesJson), locationPathname, rootPath), [to, rootPath, routePathnamesJson, locationPathname]);
}
/**
...
export function useRoutes(routes, locationArg) {
    ...
    if (__DEV__) {
        ...
        warning(matches == null ||
            matches[matches.length - 1].route.element !== undefined, `Matched leaf route at location "${location.pathname}${location.search}${location.hash}" does not have an element. ` +
...
function flattenRoutes(routes, branches = [], parentsMeta = [], parentPath = "") {
    routes.forEach((route, index) => {
        ...
        if (route.children && route.children.length > 0) {
            invariant(route.index !== true, `Index routes must not have child routes. Please remove ` +
...
function _renderMatches(matches, parentMatches = []) {
    ...
    return matches.reduceRight((outlet, match, index) => {
        return (React.createElement(RouteContext.Provider, { children: match.route.element !== undefined ? match.route.element : React.createElement(Outlet, null), value: {
                outlet,
 ...
 */
export function resolvePath(to, fromPathname = "/", rootPath = "/") {
...
function resolveTo(toArg, routePathnames, locationPathname, rootPath) {
